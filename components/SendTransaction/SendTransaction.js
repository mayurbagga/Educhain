...
const DEFAULT_PROMPT_MESSAGES = [
  ...
  {
    prompt: 'Swap 0.0001 BNB to USDC on BSC MAINNET network with 5% slippage',
    example: 'Swap 0.0001 BNB to USDC on BSC MAINNET network with 5% slippage',
    category: 'BSC'
  },
  {
    prompt:
  ...
  {
    prompt:
      'Transfer 0.01 CCIP-BnM to SEPOLIA TESTNET',
    example:
      'Transfer 0.01 CCIP-BnM to SEPOLIA TESTNET',
    category: 'CCIP'
  },
...
const SendTransaction = ({ id, initialMessages }) => {
  ...
  const sendTransactionFuncSwapModeDeployment = async (transactionCallData) => {
    ...
    try {
      ...
      // const signedTx = await web3.eth.getTransactionReceipt("0x32d7054ccc14ce9ed4e0198df4a7137d4b961cad4acd2909b5e7f55ddd73a29b");

      const deployedAddress = await web3.eth.abi.decodeParameters(["uint", "address", "address"], signedTx.logs[0].data)
      setDeployedContract(deployedAddress[2]);
      console.log("RECEIPT", signedTx, deployedAddress[2]);
      ...
      setMessages([
        // ...chatMessages,
        {
          ...
          role: 'function',
          content: `The deployed address is available at ${deployedAddress[2]}`,
          display: "displayContract_form"
        },
      ],)
  ...
  const functionCallHandler = async (chatMessages, functionCall) => {
    ...
    if (functionCall.name === 'DEPLOY_STRATEGY') {
      setAction((prev) => ({
        ...prev,
      ...
      //     name: 'DEPLOY_STRATEGY',
      //     role: 'function',
      //     content: `The deployed address is available at ${deployedContract}`,
      //     display: "displayContract_form"
      //   },
      // ],)
    }

    if (functionCall.name === 'INTERACT_STRATEGY') {
      setAction((prev) => ({
        ...prev,
        type: 'INTERACT_STRATEGY',
        data: {},
      }));
      ...
      const apiResponse = await axios.post(
        `${process.env.NEXT_PUBLIC_WEB_THREE_AGENT_NODE_APP}/api/v1/calldata/`,
        {
          ...
          userAddress: address,
          chainId: chainId,
          userStrategyAddress: parsedArguments.userStrategyAddress,
          actionType: parsedArguments.actionType,
                      ...
                      {messages?.slice(1).map((chat, index) => {
                        return (chat.content != "" && chat?.display != "none") ?
                          ...
                          <>
                            ...
                            {chat.display == "deployContract_form" &&

                              <div >

                                <div style={{ textAlign: "center" }} >

                                  <button style={{ background: "purple", textAlign: "center" }} className='btn-default btn-small btn-border' onClick={() => sendTransactionFuncSwapModeDeployment(transactionCallData)}>Deploy Vault</button>
                                </div>

                              </div>
                            }
                            {chat.display == "displayContract_form" &&
