...
export const TokenScoreComponent = ({ data: action }) => {
    ...
    useEffect(() => {
        ...
        var ctx = document?.getElementById(action.id + 'doughnutChart')?.getContext('2d');
        ...
        var barCtx = document?.getElementById(action.id + 'barChart')?.getContext('2d');
                        ...
                        <div className='card-body text-center h3 pb-0 mb-0'>Token Score</div><div className='p-4' style={{ width: '100%', height: '45vh', display: "flex", justifyContent: 'center', alignItems: 'center' }}>
                            <div className='row align-items-center' style={{ height: '100%' }}>
                                <div className='col-6' style={{ height: '100%' }}>
                                    <canvas id={action.id + 'doughnutChart'}></canvas>
                                </div>
                                <div className='col-6'>
                                    <canvas id={action.id + 'barChart'} style={{ height: '100%' }}></canvas>
                                </div>
                            </div>
                        </div>
                    </>
